...
void MeshCylinder::BuildTopCap(vector<VertexMesh>& vertices, vector<UINT>& indices)
{
	float y = 0.5f * height;
	float theta = 2.0f * Math::PI / (float)sliceCount;
...
void MeshCylinder::BuildBottomCap(vector<VertexMesh>& vertices, vector<UINT>& indices)
{
	float y = -0.5f * height;
	float theta = 2.0f * Math::PI / (float)sliceCount;
